...
#: The HTML entry points, and the built file behind each.
PAGES = {"/": "library.html", "/reader": "reader.html", "/themes": "themes.html"}
...
def create_app(registry: RecordingRegistry) -> FastAPI:
    ...
    def _page(url: str) -> FileResponse:
        """The built page for a URL."""
        path = WEB_DIR / PAGES[url]
        if not path.exists():
    ...
    async def cache_policy(request: Request, call_next):
        ...
        This replaces the blanket no-cache the unbundled frontend needed. That
        rule existed because a browser holding yesterday's ``util.js`` next to
        today's ``app.js`` did not degrade -- the import failed to link and the
        whole module graph died, with an error that named no file. A bundle has
        no such seam.
