...
#: The built interface. Generated by `npm run build` in frontend/ and committed,
#: so installing the tool from a wheel needs no Node -- the same reasoning as the
#: version ceilings in pyproject.toml: a study opened in three years should not
#: need an npm registry to still be openable.
...
def create_app(registry: RecordingRegistry) -> FastAPI:
    ...
    def _page(url: str, fallback: str) -> FileResponse:
        """The built page for a URL, or the hand-written one still standing in."""
        built = PAGES.get(url)
        if built is None:
            return FileResponse(STATIC_DIR / fallback)
        path = WEB_DIR / built
        if not path.exists():
            # A checkout with no build is a mistake with a fix, not a mystery.
            raise HTTPException(
                status_code=500,
    ...
    def reader() -> FileResponse:
        return _page("/reader", "index.html")

    @app.get("/themes")
    def themes_page() -> FileResponse:
        return _page("/themes", "themes.html")

    @app.middleware("http")
    async def cache_policy(request: Request, call_next):
        """Hold the fingerprinted assets forever; never hold the page naming them.

        The build gives every asset a name derived from its bytes, so a stale one
        is unreachable by construction and caching it for a year costs nothing.
        The HTML is the opposite -- the only file whose name never changes and
        whose contents always do -- so it is the one that must be revalidated.

        The pages still served from static/ keep the blanket no-cache they needed
        as unbundled ES modules: a browser holding yesterday's ``util.js`` next to
        today's ``app.js`` does not degrade, it fails to link and the whole module
        graph dies, with an error that names no file. A bundle has no such seam.
    ...
    # Mounted only if it is there. StaticFiles raises when its directory is
    # missing, which would take the whole app down at startup over a frontend
    # that has not been built -- and the error names a path, not the fix. Left
    # unmounted, the app starts and the page routes say what to run.
